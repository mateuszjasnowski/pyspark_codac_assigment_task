...
class TestDataJoinData:
    ...
    def test_data_join_correct_join_order(self, dataset_object):
        ...
        dataset_object.join_data(
            "./codac_app/tests/test_set/dataset_two.csv", first_pk="id", second_pk="id"
        )

        expected_result = SPARK.read.option("header", "true").csv(
            "./codac_app/tests/test_set/test_join_result_1.csv"
        )
    ...
    def test_data_join_wrong_order(self):
        ...
        test_object = Data(SPARK, "./codac_app/tests/test_set/dataset_two.csv", header=True)

        test_object.join_data(
            "./codac_app/tests/test_set/dataset_one.csv", first_pk="id", second_pk="id"
        )

        expected_result = SPARK.read.option("header", "true").csv(
            "./codac_app/tests/test_set/test_join_result_2.csv"
        )
